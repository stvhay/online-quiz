...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const showQuestion = (n) => {
        ...
        let questionHTML = `<div class='question'>${prompt ? `<h4>${prompt}</h4>` : ''}${question ? `<h2>${question}</h2>` : ''}</div>`;
        questionHTML += image ? `<img src="${image}" alt="Question Image" style="width:100%; max-width:40ex;">` : '';

        questionHTML += Object.entries(answers).map(([key, value]) =>
            `<button onclick="selectAnswer('${key}', ${n})">${value}</button>`
        ).join('<br>');
    ...
    const updateScores = (answerKey, questionIndex) => {
        const { scores } = questions[questionIndex];
        Object.entries(scores[answerKey]).forEach(([category, value]) => {
    ...
    const submitResults = async () => {
        ...
        await fetch('https://script.google.com/macros/s/YOUR_DEPLOYMENT_HERE/exec', {
            method: 'POST',
            mode: 'no-cors',
